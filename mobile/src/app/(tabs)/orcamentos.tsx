...
export default function Orcamentos(){
 ...
 const convert=async(q:Quote)=>{setError("");try{const number=`OS-${Date.now().toString().slice(-6)}`;const{error:e}=await supabase.rpc("create_service_order_from_quote",{p_quote_id:q.id,p_number:number});if(e)throw e;await load();Alert.alert("OS criada",`${number} · ${brl(q.total)}`)}catch(e){setError(e instanceof Error?e.message:"Não foi possível converter o orçamento.")}};
 const sharePdf=async(q:Quote)=>{setError("");try{const officeId=await getOfficeId();const[{data:office},{data:its}]=await Promise.all([supabase.from("offices").select("name").eq("id",officeId).single(),supabase.from("quote_items").select("description,quantity,unit_price,total").eq("quote_id",q.id).order("created_at")]);const client=clientName(q.client_id);const vehicle=vehicleName(q.vehicle_id);const rows=(its||[]).map(i=>`<tr><td>${i.description||""}</td><td>${i.quantity}</td><td>${brl(Number(i.unit_price||0))}</td><td>${brl(Number(i.total||0))}</td></tr>`).join("");const html=`<html><head><meta name="viewport" content="width=device-width,initial-scale=1"/><style>body{font-family:Arial;padding:24px;color:#172033}h1{margin:0 0 4px}h2{margin-top:28px}.muted{color:#687386}.box{padding:14px;background:#f5f7fa;border-radius:8px;margin-top:14px}table{width:100%;border-collapse:collapse;margin-top:12px}th,td{text-align:left;padding:8px;border-bottom:1px solid #ddd}th{background:#f5f7fa}.total{text-align:right;font-size:20px;font-weight:bold;margin-top:18px}</style></head><body><h1>${office?.name||"Chave 10"}</h1><div class="muted">ORÇAMENTO ${q.number}</div><div class="box"><b>Cliente:</b> ${client}<br/><b>Veículo:</b> ${vehicle||"Não informado"}<br/><b>Data:</b> ${new Date(q.created_at).toLocaleDateString("pt-BR")}</div><h2>Serviço</h2><p>${q.description||""}</p><h2>Peças</h2>${rows?`<table><tr><th>Descrição</th><th>Qtd.</th><th>Unitário</th><th>Total</th></tr>${rows}</table>`:"<p>Nenhuma peça detalhada.</p>"}<p>Mão de obra: ${brl(q.labor)}</p><p>Peças: ${brl(q.parts)}</p><p>Desconto: ${brl(q.discount)}</p><div class="total">Total: ${brl(q.total)}</div></body></html>`;const file=await Print.printToFileAsync({html});if(await Sharing.isAvailableAsync())await Sharing.shareAsync(file.uri,{mimeType:"application/pdf",dialogTitle:`Orçamento ${q.number}`});else await Print.printAsync({html});}catch(e){setError(e instanceof Error?e.message:"Não foi possível gerar o PDF.")}};
 const whatsapp=async(q:Quote)=>{const text=encodeURIComponent(`Olá! Segue o orçamento ${q.number}.\\nTotal: ${brl(q.total)}\\n\\nServiço: ${q.description||""}`);const url=`https://wa.me/?text=${text}`;try{await Linking.openURL(url)}catch(e){setError("Não foi possível abrir o WhatsApp.")}};
 ...
 const save=async()=>{
  ...
  try{
   ...
   const rows=quoteItems.filter(i=>i.product_id&&money(i.quantity)>0).map(i=>{const p=products.find(x=>x.id===i.product_id)!;const qty=money(i.quantity);return{quote_id:q.id,product_id:p.id,description:p.description,quantity:qty,unit_price:Number(p.unit_price||0),total:qty*Number(p.unit_price||0)}});
